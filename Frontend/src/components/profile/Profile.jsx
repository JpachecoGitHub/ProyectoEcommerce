...
const Profile = () => {
  const { user, historialCompra, loading, logout, uploadProfileImage, updateProfile } = useContext(UserContext)
  ...
  const handleRebuy = (item) => {
    ...
    Swal.fire({
      ...
      text: `"${item.nombre}" ha sido agregado de nuevo a tu carrito.`,
    ...
    <div className='container-fluid bg-light p-0'>
      <main className='container my-5'>
        ...
        <section className='bg-white p-4 rounded shadow-sm'>
          ...
          <hr />
          {historialCompra && historialCompra.length > 0
            ? (
                historialCompra.map(item => (
                  <div key={item.id} className='d-flex align-items-center border-bottom py-3'>
                    <div className='me-4 bg-light border' style={{ width: '80px', height: '80px' }}>
                      <img src={item.imagen_url} alt={item.nombre} className='img-fluid' />
                    </div>
                    ...
                    <div className='text-center me-4'>
                      <strong className='d-block'>${item.precio}</strong>
                    </div>
                    <div className='text-center me-4'>
                      <small>x{item.quantity}</small>
                    </div>
                    <Button variant='dark' onClick={() => handleRebuy(item)}>
                      Comprar de Nuevo
                    </Button>
                  </div>
                ))
