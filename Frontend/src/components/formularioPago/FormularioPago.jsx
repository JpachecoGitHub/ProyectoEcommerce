  ...
  <li className='list-group-item d-flex justify-content-between align-items-center p-3 mb-2 bg-light rounded shadow-sm'>
    <div className='d-flex align-items-center'>
      ...
      <div>
        ...
        <p className='mb-0 text-secondary small'>{item.descripcion || 'Producto'}</p>
      </div>
    </div>
    <span className='fw-bold text-success'>${(item.precio * item.quantity).toLocaleString('es-CL')}</span>
  </li>
)

const FormularioPago = () => {
  const { cart, calcularTotal, vaciarCart } = useContext(CartContext)
  const { user, recordCompra } = useContext(UserContext)
  ...
  const handleSubmit = (e) => {
    ...
    Swal.fire({
      ...
      if (result.dismiss === Swal.DismissReason.timer || result.isConfirmed) {
        ...
        // Llamo a recordCompra para registrar el pedido y actualizar el historial
        const purchaseSuccess = await recordCompra(cart, shippingDetails)
        ...
        // sendEmail()
          ...
          Swal.fire('Atención', 'El pago fue procesado, pero hubo un error al registrar el pedido.', 'warning')
        }
      }
    })
  }
